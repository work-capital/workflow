defmodule Engine.Storage.PostgresTest do
  use ExUnit.Case
  require Logger
  alias Engine.Storage.Eventstore
  alias Engine.Storage.Storage
  @storage Engine.Storage.Postgres   # we use this value to override the default settings
  ...
  #   {:ok, position}   = Storage.append_event(id ,%PersonCreated{name: "jim"},     @storage)
  #   {:ok, position2}  = Storage.append_event(id ,%PersonChangedName{name: "joe"}, @storage)
  #   #IO.inspect message
